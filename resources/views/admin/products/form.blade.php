...
<div class="d-flex justify-content-between align-items-center flex-wrap gap-3 mb-4">
    ...
    <a href="{{ !empty($returnUrl) ? $returnUrl : route('admin.products.index') }}" class="btn btn-light rounded-pill px-4">กลับรายการสินค้า</a>
</div>
...
<div class="sticky-action-bar">
    <div class="d-flex gap-2 flex-wrap"><a href="{{ !empty($returnUrl) ? $returnUrl : route('admin.products.index') }}" class="btn btn-outline-secondary rounded-pill px-4">กลับรายการสินค้า</a></div>
    <div class="d-flex gap-2"><a href="{{ !empty($returnUrl) ? $returnUrl : route('admin.products.index') }}" class="btn btn-light rounded-pill px-4">ยกเลิก</a><button class="btn btn-danger rounded-pill px-5 fw-bold">บันทึกข้อมูล</button></div>
</div>
</form>
@if(isset($product))
    @foreach($product->images as $img)
