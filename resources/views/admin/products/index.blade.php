...
@endphp
<tr id="product-row-{{$p->id}}" data-product-row="{{$p->id}}">
    ...
    <td class="text-end text-nowrap">
        <a class="btn btn-sm btn-outline-primary rounded-pill px-3 product-edit-link" href="{{route('admin.products.edit', [$p, 'return_url' => request()->fullUrl()])}}">แก้ไข</a>
...
(function(){
  ...
  if(window.location.hash && window.location.hash.startsWith('#product-row-')){ const row=document.querySelector(window.location.hash); if(row){ setTimeout(()=>{ row.scrollIntoView({behavior:'smooth', block:'center'}); row.classList.add('product-row-highlight'); setTimeout(()=>row.classList.remove('product-row-highlight'),2600); },160); } }
})();
