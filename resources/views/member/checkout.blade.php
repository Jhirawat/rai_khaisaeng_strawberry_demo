        ...
        <div class="row g-4">
            ...
            <div class="col-lg-5">
                <div class="bg-white rounded-4 shadow-sm p-4 sticky-top checkout-summary-card" style="top:100px">
                    ...
                    <div class="d-flex justify-content-between"><span>{{__('Product Total')}}</span><b>฿{{number_format($subtotal,2)}}</b></div>
                    <div class="d-flex justify-content-between mt-2"><span>{{__('Shipping Fee')}}<div class="small text-muted">{{$shippingNote}}</div></span><b>฿{{number_format($shipping,2)}}</b></div><hr>
                    <div class="d-flex justify-content-between"><span>{{__('Amount before VAT')}}</span><span>฿{{number_format($beforeVat,2)}}</span></div>
                    <div class="d-flex justify-content-between mt-1"><span>{{__('VAT')}} {{$vatRate}}%</span><span>฿{{number_format($vat,2)}}</span></div>
