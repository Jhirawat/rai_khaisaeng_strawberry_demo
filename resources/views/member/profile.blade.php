        ...
        <div class="col-lg-9">
            <div id="account" class="bg-white rounded-4 shadow-sm p-4 mb-4">
                ...
                <form method="post" action="{{route('member.profile.update')}}" class="row g-3">
                    ...
                    <div class="col-md-6"><label class="form-label fw-bold">{{__('Phone')}}ศัพท์</label><input class="form-control rounded-pill" name="phone" value="{{old('phone',$user->phone)}}" placeholder="เช่น 0899998295" inputmode="numeric" pattern="[0-9]{9,10}" maxlength="10" oninput="this.value=this.value.replace(/[^0-9]/g,'').slice(0,10)"></div>
                    <div class="col-md-6"><label class="form-label fw-bold">รหัสผ่านใหม่</label><input type="password" class="form-control rounded-pill" name="password" placeholder="เว้นว่างไว้หากไม่เปลี่ยน"></div>
                    <div class="col-md-6"><label class="form-label fw-bold">ยืนยันรหัสผ่านใหม่</label><input type="password" class="form-control rounded-pill" name="password_confirmation"></div>
