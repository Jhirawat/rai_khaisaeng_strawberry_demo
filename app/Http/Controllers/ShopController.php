...
use App\Models\{Product,Category,Promotion}; use Illuminate\Http\Request;
class ShopController extends Controller{
 public function home(){ $featured=Product::with('category','inventory','images')->where('featured',1)->take(8)->get(); $categories=Category::where('is_active',1)->get(); $homePromotion=Promotion::active()->where('position','home_banner')->orderBy('sort_order')->latest()->first(); return view('shop.home',compact('featured','categories','homePromotion')); }
