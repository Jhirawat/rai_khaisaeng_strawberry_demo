...
class OrderController extends Controller
{
    ...
    public function show(Order $order){ $order->load('items.product','user','payment','shipment'); return view('admin.orders.show',compact('order')); }
    public function update(Request $r,Order $order)
    {
        ...
        DB::transaction(function() use($order,$d){
            ...
            if ($d['status']==='cancelled') $this->returnStockOnce($order);
        });
    ...
    public function bulkUpdate(Request $request)
    {
        ...
        foreach ($data['statuses'] as $orderId => $status) {
            ...
            DB::transaction(function() use($order,$status){
                ...
                if ($status==='cancelled') $this->returnStockOnce($order);
            });
    ...
    {
        if ($order->stock_returned_at) return;
        $order->loadMissing('items.product.inventory');
        foreach ($order->items as $item) {
            if ($item->product && $item->product->inventory) $item->product->inventory->increment('quantity', (int) $item->quantity);
        }
